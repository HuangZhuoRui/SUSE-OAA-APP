...
### 变更
- 🔄 **接入 v2 后端**（`https://v2-api.suseoaa.com/v2`），协会相关接口全部迁移
- 🔐 登录支持学号 / 用户名 / 邮箱；改用 refresh token 自动续期，不再在本地保存协会账号的明文密码
- 🔑 修改密码改为「当前密码 + 新密码」；忘记密码仍走邮箱验证码
- 🏠 首页部门改为从后端动态获取；公告支持标题、草稿、发布、历史记录与删除
- 👥 成员管理支持分页、按部门 / 职位筛选、批量调整与删除，权限改为按职位等级判断
- 📝 招新换届重做：按周期填写与修改申请、志愿选择部门 + 职位、查看录取结果；新增面试审核与周期管理

### 新增
- 🏛️ 组织架构管理（部门、职位的新建、编辑与停用），需 level ≥ 80

### 注意
- 升级后需要重新登录一次（v1 的登录态在 v2 后端上无效）
